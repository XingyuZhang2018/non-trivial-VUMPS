...
#     @show X1 X2 X1sq X2sq
    ...
    @tensor MT[a,b,c,d] := I[a',b',c',d']*Xsq[1][a',a]*Xsq[2][b',b]*Xsq[2][c',c]*Xsq[1][d',d]
    M[1,1] = MT
    ...
    ME_row = Array{Array,2}(undef, 2, 2)
    ME_row[1,1] = me_row(Xsq,Y,I,[1,2,2,1,1,1,2])
    ME_row[1,2] = me_row(Xsq,Y,I,[2,2,2,2,1,1,1])
    ME_row[2,1] = me_row(Xsq,Y,I,[1,1,1,1,2,2,2])
    ME_row[2,2] = me_row(Xsq,Y,I,[2,1,1,2,2,2,1])
    
    ME_col = Array{Array,2}(undef, 2, 2)
    ME_col[1,1] = me_col(Xsq,Y,I,[1,1,1,2,2,1,2])
    ME_col[1,2] = me_col(Xsq,Y,I,[2,2,1,1,1,1,2])
    ME_col[2,1] = me_col(Xsq,Y,I,[1,1,2,2,2,2,1])
    ME_col[2,2] = me_col(Xsq,Y,I,[2,2,2,1,1,2,1])
